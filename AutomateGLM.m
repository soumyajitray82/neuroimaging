...
for taskInd=5 %:size(tasks,1) 
    for typeInd=1:size(types,1) % Cycle through 'test' and 'retest' data
        for condInd=1:taskCond(taskInd)
            ...
            fprintf(fid,['matlabbatch{1}.spm.stats.factorial_design.dir = {''' dataPath 'MGroupStats-' types{typeInd} '_' tasks{taskInd} '_' num2str(condInd) '''};']);
            ...
            fprintf(fid,['matlabbatch{1}.spm.stats.fmri_est.spmmat = {''' dataPath 'MGroupStats-' types{typeInd} '_' tasks{taskInd} '_' num2str(condInd) '/SPM.mat''};']);
