...
for taskInd=5 %:size(tasks,1) 
    for condInd=1:taskCond(taskInd)
        ...
        fprintf(fid,['matlabbatch{1}.spm.stats.factorial_design.dir = {''' dataPath 'MGroupStatsTR_MR' '_' tasks{taskInd} '_' num2str(condInd) '''};\n']);
       
        for subInd=1:size(subject,1)
            fprintf(fid,['matlabbatch{1}.spm.stats.factorial_design.des.pt.pair(' num2str(subInd) ').scans = {\n']);
            fprintf(fid,['''' dataPath subject(subInd,:) '/ses-test/MMR' tasks{taskInd} '/con_000' num2str(condInd) '.nii,1''\n']);
            fprintf(fid,['''' dataPath subject(subInd,:) '/ses-retest/MMR' tasks{taskInd} '/con_000' num2str(condInd) '.nii,1''\n']);
            fprintf(fid,'};\n');
        end                                            
        ...
        fprintf(fid,['matlabbatch{1}.spm.stats.fmri_est.spmmat = {''' dataPath 'MGroupStatsTR_MR' '_' tasks{taskInd} '_' num2str(condInd) '/SPM.mat''};']);
