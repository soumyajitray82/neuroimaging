...
for taskInd=5 % 1:size(tasks,1) 
    for subjInd=1:size(subject,1) % Cycle through all the subjects
        for typeInd=1:size(types,1) % Cycle through 'test' and 'retest' data
            ...
            fprintf(fid,'matlabbatch{1}.spm.stats.fmri_spec.sess.multi_reg = {''''};\n');
            ...
            fprintf(fid,['matlabbatch{1}.spm.stats.fmri_est.spmmat = {''' dataPath subject(subjInd,:) '/ses-' types{typeInd} '/M' tasks{taskInd} '/SPM.mat''};']);
...
for taskInd=5 %:size(tasks,1) 
    for subjInd=1:size(subject,1) % Cycle through all the subjects
        for typeInd=1:size(types,1) % Cycle through 'test' and 'retest' data
            ...
            fprintf(fid,['matlabbatch{1}.spm.stats.con.spmmat = {''' dataPath subject(subjInd,:) '/ses-' types{typeInd} '/M' tasks{taskInd} '/SPM.mat''};']);
